...
// Require admin login.
admin_externalpage_setup('local_aiquiz_gen_admin');

$context = context_system::instance();
require_capability('moodle/site:config', $context);

// Page setup.
$PAGE->set_url(new moodle_url('/local/aiquiz_gen/admin_dashboard.php'));
$PAGE->set_context($context);
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('admin_dashboard_title', 'local_aiquiz_gen'));
$PAGE->set_heading(get_string('admin_dashboard_heading', 'local_aiquiz_gen'));
...
$totalquestionsgenerated = $DB->count_records('local_aiquiz_gen_questions');
$totalquizzescreated = $DB->count_records('local_aiquiz_gen_requests', ['status' => 'completed']);

$activeteachers = $DB->count_records_sql(
    "SELECT COUNT(DISTINCT userid) FROM {local_aiquiz_gen_requests}",
    []
);

$activecourses = $DB->count_records_sql(
    "SELECT COUNT(DISTINCT courseid) FROM {local_aiquiz_gen_requests}",
    []
);
...
$totalapproved = $DB->count_records_sql(
    "SELECT COUNT(*) FROM {local_aiquiz_gen_questions}
...
$totaluserswithcapability = $DB->count_records_sql(
    "SELECT COUNT(DISTINCT ra.userid)
     ...
     WHERE rc.capability = ?
     AND rc.permission = 1",
    ['local/aiquiz_gen:generatequestions']
);
...
$rawusagedata = $DB->get_records_sql(
    "SELECT timecreated FROM {local_aiquiz_gen_questions} WHERE timecreated >= ?",
    [$thirtydaysago]
);
...
$pendinggenerations = $DB->count_records('local_aiquiz_gen_requests', ['status' => 'pending']);
$failedgenerations = $DB->count_records('local_aiquiz_gen_requests', ['status' => 'failed']);

// Recent errors (last 7 days).
$sevendaysago = time() - (7 * 24 * 60 * 60);
$recenterrors = $DB->count_records_sql(
    "SELECT COUNT(*) FROM {local_aiquiz_gen_requests}
...
$PAGE->requires->js_call_amd('local_aiquiz_gen/admindashboard', 'init', [[
    ...
    'strings' => [
        'questionsGenerated' => get_string('chart_questions_generated', 'local_aiquiz_gen'),
        'activeTeachers' => get_string('chart_active_teachers', 'local_aiquiz_gen'),
        'inactiveTeachers' => get_string('chart_inactive_teachers', 'local_aiquiz_gen'),
        'totalUsers' => get_string('chart_total_users', 'local_aiquiz_gen'),
        'teachersSuffix' => get_string('chart_teachers_suffix', 'local_aiquiz_gen'),
...
$page = new \local_aiquiz_gen\output\admin_dashboard_page(
    [
...
echo $OUTPUT->render_from_template('local_aiquiz_gen/admin_dashboard', $page->export_for_template($OUTPUT));

// Charts handled by AMD module local_aiquiz_gen/admindashboard.
