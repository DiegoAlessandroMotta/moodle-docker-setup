...
// Page setup.
$PAGE->set_url(new moodle_url('/local/aiquiz_gen/analytics.php', ['courseid' => $courseid]));
$PAGE->set_context($context);
$PAGE->set_course($course);
$PAGE->set_pagelayout('incourse');
$PAGE->set_title(get_string('pluginname', 'local_aiquiz_gen') . ' - ' . get_string('analytics_title', 'local_aiquiz_gen'));
...
$PAGE->requires->js(new moodle_url('/local/aiquiz_gen/apexcharts.js'), true);
 ...
 */
function local_aiquiz_gen_get_filtered_sql($basesql, $baseparams, $timefilter, $timefield = 'timecreated') {
    if ($timefilter > 0) {
...
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT COUNT(*) FROM {local_aiquiz_gen_questions} WHERE userid = ? AND courseid = ?",
    [$userid, $courseid],
    $timefilter
);
...
// Approved questions.
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT COUNT(*) FROM {local_aiquiz_gen_questions} WHERE userid = ? AND courseid = ? AND status IN ('approved', 'deployed')",
    [$userid, $courseid],
    $timefilter
);
...
// Rejected questions.
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT COUNT(*) FROM {local_aiquiz_gen_questions} WHERE userid = ? AND courseid = ? AND status = 'rejected'",
    [$userid, $courseid],
    $timefilter
);
...
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT AVG(validation_score) FROM {local_aiquiz_gen_questions}
...
// Total regenerations.
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT SUM(regeneration_count) FROM {local_aiquiz_gen_questions} WHERE userid = ? AND courseid = ?",
    [$userid, $courseid],
    $timefilter
);
...
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT COUNT(*) FROM {local_aiquiz_gen_requests} WHERE userid = ? AND courseid = ?",
    [$userid, $courseid],
    $timefilter
);
...
[$sql, $params] = local_aiquiz_gen_get_filtered_sql(
    "SELECT questiontype, COUNT(*) as count,
     ...
     FROM {local_aiquiz_gen_questions}
...
$PAGE->requires->js_call_amd('local_aiquiz_gen/analytics', 'init', [[
    'courseid' => $courseid,
    'sesskey' => sesskey(),
    'timerange' => $timerange,
    'stats' => [
    ...
    'strings' => [
        'questions' => get_string('chart_questions', 'local_aiquiz_gen'),
        'generated' => get_string('chart_generated', 'local_aiquiz_gen'),
        'reviewed' => get_string('chart_reviewed', 'local_aiquiz_gen'),
        'approved' => get_string('chart_approved', 'local_aiquiz_gen'),
        'deployed' => get_string('chart_deployed', 'local_aiquiz_gen'),
        'total' => get_string('chart_total', 'local_aiquiz_gen'),
        'qualityScoreRange' => get_string('chart_quality_score_range', 'local_aiquiz_gen'),
        'percentageOfQuestions' => get_string('chart_percentage_of_questions', 'local_aiquiz_gen'),
        'numberOfQuestions' => get_string('chart_number_of_questions', 'local_aiquiz_gen'),
        'numberOfRegenerations' => get_string('chart_number_of_regenerations', 'local_aiquiz_gen'),
        'percentage' => get_string('chart_percentage', 'local_aiquiz_gen'),
        'avgRegenerations' => get_string('chart_avg_regenerations', 'local_aiquiz_gen'),
    ],
]]);
...
$page = new \local_aiquiz_gen\output\analytics_page(
    $courseid,
    $timerange,
    [
...
echo $OUTPUT->render_from_template('local_aiquiz_gen/analytics', $page->export_for_template($OUTPUT));
