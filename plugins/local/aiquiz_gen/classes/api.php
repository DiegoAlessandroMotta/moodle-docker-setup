...
class api {
    ...
    public static function update_request_status(int $requestid, string $newstatus, ?string $errormessage = null): bool {
        ...
        if (!isset(self::REQUEST_STATUSES[$oldstatus])) {
            throw new \moodle_exception('error:invalidstatus', 'local_aiquiz_gen', '', $oldstatus);
        }
        ...
        $result = $DB->update_record('local_aiquiz_gen_requests', $update);
    ...
    public static function create_request(int $courseid, array $contentsources, array $config = []): int {
        ...
        $request->processing_mode = $config['processing_mode'] ?? get_config('local_aiquiz_gen', 'default_quality_mode');
        ...
        $requestid = $DB->insert_record('local_aiquiz_gen_requests', $request);
    ...
    public static function get_questions(int $requestid, ?string $status = null): array {
        ...
        $questions = $DB->get_records('local_aiquiz_gen_questions', $params, 'timecreated ASC');
        ...
        foreach ($questions as $question) {
            ...
            if (!empty($question->quality_rating)) {
                $question->quality_badge = get_string('quality_' . $question->quality_rating, 'local_aiquiz_gen');
            }
        }
    ...
    public static function update_question(int $questionid, array $data): bool {
        ...
        $question = $DB->get_record('local_aiquiz_gen_questions', ['id' => $questionid], '*', MUST_EXIST);
        ...
        $result = $DB->update_record('local_aiquiz_gen_questions', $record);
    ...
    public static function delete_question(int $questionid): bool {
        ...
        // Delete answers.
        $DB->delete_records('local_aiquiz_gen_answers', ['questionid' => $questionid]);

        // Delete question.
        $result = $DB->delete_records('local_aiquiz_gen_questions', ['id' => $questionid]);
    ...
    public static function log_action(
        ...
        $log->action = $action;
        $log->component = 'local_aiquiz_gen';
        $log->details = json_encode($details);
        $log->status = $status;
        $log->error_message = $error;
        $log->timecreated = time();

        $DB->insert_record('local_aiquiz_gen_logs', $log);
    }
    ...
    public static function get_request_history(int $courseid, ?int $userid = null): array {
        ...
        return $DB->get_records('local_aiquiz_gen_requests', $params, 'timecreated DESC', '*', 0, 500);
    }
    ...
    public static function get_statistics(?int $courseid = null): array {
        ...
        if ($courseid) {
            ...
            // Total requests.
            $stats['total_requests'] = $DB->count_records('local_aiquiz_gen_requests', ['courseid' => $courseid]);

            // Total questions.
            $stats['total_questions'] = $DB->count_records_sql(
                "SELECT COUNT(*) FROM {local_aiquiz_gen_questions} q
                 JOIN {local_aiquiz_gen_requests} r ON q.requestid = r.id
            ...
            // Total requests.
            $stats['total_requests'] = $DB->count_records('local_aiquiz_gen_requests');
    ...
    public static function diagnose_deployment(int $requestid): array {
        ...
        $pluginquestions = $DB->get_records('local_aiquiz_gen_questions', ['requestid' => $requestid]);
    ...
    public static function regenerate_question(int $questionid): \stdClass {
        ...
        if ($question->userid != $USER->id && !has_capability('local/aiquiz_gen:editquestions', $context)) {
        ...
        $maxregenerations = get_config('local_aiquiz_gen', 'max_regenerations') ?: 5;
        ...
        $topic = $DB->get_record('local_aiquiz_gen_topics', ['id' => $question->topicid], '*', MUST_EXIST);

        // Get request for config.
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $question->requestid], '*', MUST_EXIST);
        ...
        try {
            ...
            $newquestions = \local_aiquiz_gen\question_generator::generate_for_topic(
                $question->topicid,
                $request->id,
                $config
            );
            ...
            if (empty($newquestions)) {
                throw new \moodle_exception('error:questiongeneration', 'local_aiquiz_gen');
            }
            ...
            $DB->delete_records('local_aiquiz_gen_answers', ['questionid' => $questionid]);
            $DB->delete_records('local_aiquiz_gen_questions', ['id' => $questionid]);

            // Update new question's timecreated to match original position.
            $DB->set_field('local_aiquiz_gen_questions', 'timecreated', $originaltimecreated, ['id' => $newquestion->id]);
            ...
            $columns = $DB->get_columns('local_aiquiz_gen_questions');
