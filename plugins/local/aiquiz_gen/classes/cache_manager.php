...
class cache_manager {
    ...
    public static function get_cached_response($cachetype, $cachekey) {
        ...
        try {
            $cache = $DB->get_record('local_aiquiz_gen_cache', [
                'cachetype' => $cachetype,
                'cachekey' => $cachekey,
            ]);
            ...
            $DB->update_record('local_aiquiz_gen_cache', $cache);
    ...
    public static function set_cached_response($cachetype, $cachekey, $data, $metadata = []) {
        ...
        try {
            ...
            $existing = $DB->get_record('local_aiquiz_gen_cache', [
                'cachetype' => $cachetype,
                'cachekey' => $cachekey,
            ]);
            ...
            if ($existing) {
                ...
                $DB->update_record('local_aiquiz_gen_cache', $record);
            } else {
                $record->timecreated = time();
                $record->hits = 0;
                $DB->insert_record('local_aiquiz_gen_cache', $record);
            }
    ...
    public static function delete_cache($cachetype, $cachekey) {
        ...
        try {
            $DB->delete_records('local_aiquiz_gen_cache', [
                'cachetype' => $cachetype,
                'cachekey' => $cachekey,
            ]);
            return true;
    ...
    public static function cleanup_expired_cache() {
        ...
        $deleted = $DB->count_records_select('local_aiquiz_gen_cache', $where, $params);
        if ($deleted > 0) {
            $DB->delete_records_select('local_aiquiz_gen_cache', $where, $params);
        }
    ...
    public static function get_cache_statistics() {
        ...
        $sql = "SELECT SUM(LENGTH(data)) as total_size FROM {local_aiquiz_gen_cache}";
    ...
    public static function clear_all_cache($cachetype = null) {
        ...
        try {
            if ($cachetype) {
                $DB->delete_records('local_aiquiz_gen_cache', ['cachetype' => $cachetype]);
            } else {
                $DB->delete_records('local_aiquiz_gen_cache');
            }
            return true;
    ...
    public static function is_caching_enabled() {
        return (bool) get_config('local_aiquiz_gen', 'enable_caching');
    }
