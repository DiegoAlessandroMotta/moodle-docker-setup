...
class content_extractor {
    ...
    public static function extract_from_file(string $filepath, string $originalfilename = ''): array {
        if (!file_exists($filepath)) {
            throw new \moodle_exception('error:contentextraction', 'local_aiquiz_gen', '', 'File not found');
        }
        ...
        switch ($extension) {
            ...
            case 'txt':
                ...
                break;
            default:
                throw new \moodle_exception('error:invalidfiletype', 'local_aiquiz_gen');
        }
    ...
    private static function extract_pdf_pdftotext(string $filepath): string {
        ...
        $pdftotext = get_config('local_aiquiz_gen', 'pathtopdftotext');
        if (empty($pdftotext) || !is_executable($pdftotext)) {
            return '';
        }

        try {
            $tempdir = make_temp_directory('local_aiquiz_gen');
    ...
    private static function extract_pdf_ghostscript(string $filepath): string {
        ...
        $gs = get_config('local_aiquiz_gen', 'pathtogs');
    ...
    public static function extract_from_url(string $url, int $maxlength = 50000): array {
        ...
        if (!filter_var($url, FILTER_VALIDATE_URL)) {
            throw new \moodle_exception('error:invalidurl', 'local_aiquiz_gen');
        }
    ...
    public static function extract_from_activity(int $cmid, string $moduletype, int $courseid): array {
        ...
        if (!$cm) {
            throw new \moodle_exception('error:invalidactivity', 'local_aiquiz_gen');
        }
        ...
        switch ($moduletype) {
            ...
            case 'label':
                ...
                break;
            default:
                throw new \moodle_exception('error:unsupportedmodule', 'local_aiquiz_gen', '', $moduletype);
        }
    ...
    public static function validate_file_upload(array $file): bool {
        $maxsize = get_config('local_aiquiz_gen', 'max_file_size_mb') ?: 50;
        $maxbytes = $maxsize * 1024 * 1024;

        if ($file['size'] > $maxbytes) {
            throw new \moodle_exception('error:filetoobig', 'local_aiquiz_gen', '', $maxsize);
        }
    ...
    public static function extract_from_activities(int $courseid, array $cmids): string {
        ...
        debugging("AI extract_from_activities START: {$totalactivities} activities to process", DEBUG_DEVELOPER);
        ...
        foreach ($cmids as $cmid) {
            ...
            try {
                ...
                debugging("AI [{$activityindex}/{$totalactivities}] STARTING cmid={$cmid} type={$moduletype}", DEBUG_DEVELOPER);
                ...
                $processed++;
                debugging(
                    "AI [{$activityindex}/{$totalactivities}] DONE cmid={$cmid} type={$moduletype}" .
                ...
                $actduration = round(microtime(true) - $actstart, 2);
                debugging(
                    "AI [{$activityindex}/{$totalactivities}] FAILED cmid={$cmid} in {$actduration}s: " .
                    $e->getMessage(),
                    DEBUG_DEVELOPER
                );
                continue;
        ...
        debugging(
            "AI extract_from_activities DONE: {$processed}/{$totalactivities} succeeded" .
    ...
    private static function extract_from_scorm(\stdClass $cm): string {
        ...
        try {
            ...
            $tempdir = make_temp_directory('aiquiz_scorm_' . $cm->id);
            ...
            $extractdir = make_temp_directory('local_aiquiz_gen/extracted_' . uniqid());
