...
class debug_logger {
    ...
    const LOG_FILE = 'local_aiquiz_gen_debug.log';
    ...
    private static function init(): void {
        ...
        if (self::$enabled === null) {
            ...
            $logdir = make_temp_directory('local_aiquiz_gen/logs');
    ...
    public static function log(string $level, string $message, array $context = [], ?int $requestid = null): void {
        ...
        if (!empty($context)) {
            ...
            $logentry .= '    ' . get_string('log_context_label', 'local_aiquiz_gen') . ' '
                . str_replace("\n", "\n    ", $contextstr) . "\n";
        }
        ...
        if ($level === self::LEVEL_CRITICAL) {
            debugging("AI Quiz Gen CRITICAL: {$message}");
        }
    }
    ...
    private static function writetodatabase(
        ...
        try {
            ...
            $DB->insert_record('local_aiquiz_gen_logs', $record);
        } catch (\Exception $e) {
            // Silently fail to prevent cascading errors.
            debugging("AI Quiz Gen: Failed to write to log database: " . $e->getMessage());
        }
    }
