...
class error_handler {
    ...
    public static function get_error_message(string $errorkey, $a = null): string {
        if (get_string_manager()->string_exists($errorkey, 'local_aiquiz_gen')) {
            return get_string($errorkey, 'local_aiquiz_gen', $a);
        }
        return get_string('error:unknown', 'local_aiquiz_gen');
    }
    ...
    public static function validate_request_state(int $requestid, bool $allowcompleted = false): bool {
        ...
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $requestid], 'status', MUST_EXIST);

        // FIX: Allow completed requests when regenerating questions.
        if ($request->status === 'completed' && !$allowcompleted) {
            throw new \moodle_exception('error:requestalreadycompleted', 'local_aiquiz_gen');
        }

        if ($request->status === 'failed') {
            throw new \moodle_exception('error:requestfailed', 'local_aiquiz_gen');
        }
