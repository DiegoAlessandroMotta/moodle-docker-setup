...
/**
 * Diagnostic external functions for the local_aiquiz_gen plugin.
 *
...
class diagnostic_external extends external_api {
    ...
    public static function fix_category($courseid) {
        ...
        require_capability('local/aiquiz_gen:generatequestions', $coursecontext);
        ...
        if (!empty($questions)) {
            ...
            try {
                ...
                $sampledata = [
                    'id' => $sample->id,
                    'category' => $sample->category ?? get_string('ajax_not_set', 'local_aiquiz_gen'),
                    'qtype' => $sample->qtype,
                ];
            } catch (\Exception $e) {
    ...
    public static function repair_questions($courseid) {
        ...
        if (!$hascategorycolumn) {
            ...
            $message = get_string('ajax_no_category_column', 'local_aiquiz_gen');
    ...
    public static function diagnose($requestid = 0, $courseid = 0) {
        ...
        if (!$requestid && !$courseid) {
            throw new \invalid_parameter_exception(
                get_string('ajax_requestid_or_courseid_required', 'local_aiquiz_gen')
            );
        }
        ...
        if ($requestid) {
            ...
            if (!$request) {
                throw new \moodle_exception('ajax_request_not_found', 'local_aiquiz_gen');
            }
            ...
            $diagnostic = \local_aiquiz_gen\api::diagnose_deployment($requestid);
        } else {
            ...
            $requests = $DB->get_records('local_aiquiz_gen_requests', ['courseid' => $courseid], 'id DESC', 'id', 0, 5);
            ...
            foreach ($requests as $req) {
                $diagnostic['request_diagnostics'][$req->id] = \local_aiquiz_gen\api::diagnose_deployment($req->id);
            }
        }
