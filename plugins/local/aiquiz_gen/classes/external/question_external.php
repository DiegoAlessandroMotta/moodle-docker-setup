...
class question_external extends external_api {
    ...
    public static function update_question($questionid, $field, $value) {
        ...
        require_capability('local/aiquiz_gen:editquestions', $context);

        // Validate the field is allowed.
        $allowedfields = ['questiontext', 'difficulty', 'blooms_level', 'generalfeedback'];
        if (!in_array($field, $allowedfields)) {
            throw new \moodle_exception('ajax_invalid_field', 'local_aiquiz_gen');
        }
        ...
        // Perform the update.
        $DB->set_field('local_aiquiz_gen_questions', $field, $value, ['id' => $questionid]);
        $DB->set_field('local_aiquiz_gen_questions', 'timemodified', time(), ['id' => $questionid]);
    ...
    public static function update_answer($answerid, $field, $value) {
        ...
        $answer = $DB->get_record('local_aiquiz_gen_answers', ['id' => $answerid], '*', MUST_EXIST);
        $question = $DB->get_record('local_aiquiz_gen_questions', ['id' => $answer->questionid], '*', MUST_EXIST);
        ...
        // Perform the update.
        $DB->set_field('local_aiquiz_gen_answers', $field, $value, ['id' => $answerid]);
    ...
    public static function reorder_answers($questionid, $order) {
        ...
        if (!is_array($order)) {
            throw new \moodle_exception('ajax_invalid_order_format', 'local_aiquiz_gen');
        }

        // Update sortorder for each answer.
        foreach ($order as $sortorder => $answerid) {
            $DB->set_field('local_aiquiz_gen_answers', 'sortorder', $sortorder, [
                'id' => (int)$answerid,
                'questionid' => $questionid,
            ]);
        }
    ...
    public static function approve_question($questionid, $confidence = 0) {
        ...
        $context = \context_course::instance($request->courseid);
        self::validate_context($context);
        require_capability('local/aiquiz_gen:approvequestions', $context);
        ...
        if ($confidence > 0 && $confidence <= 5) {
            $DB->insert_record('local_aiquiz_gen_logs', [
    ...
    public static function bulk_approve($questionids) {
        ...
        if (!is_array($questionids) || empty($questionids)) {
            throw new \moodle_exception('ajax_no_questions_specified', 'local_aiquiz_gen');
        }

        // Validate context using the first question's course.
        $firstqid = (int)reset($questionids);
        $firstquestion = $DB->get_record('local_aiquiz_gen_questions', ['id' => $firstqid], '*', MUST_EXIST);
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $firstquestion->requestid], '*', MUST_EXIST);
        $context = \context_course::instance($request->courseid);
        self::validate_context($context);
        require_capability('local/aiquiz_gen:approvequestions', $context);
        ...
        if (!empty($ownedids)) {
            ...
            $DB->set_field_select('local_aiquiz_gen_questions', 'status', 'approved', "id " . $insql, $inparams);
            $DB->set_field_select('local_aiquiz_gen_questions', 'timemodified', $now, "id " . $insql, $inparams);
        }
    ...
    public static function bulk_reject($questionids, $reason = '') {
        ...
        if (!empty($ownedids)) {
            [$insql, $inparams] = $DB->get_in_or_equal($ownedids, SQL_PARAMS_NAMED);
            $DB->set_field_select('local_aiquiz_gen_questions', 'status', 'rejected', "id " . $insql, $inparams);
            $DB->set_field_select('local_aiquiz_gen_questions', 'timemodified', $now, "id " . $insql, $inparams);
        }
