...
class template_external extends external_api {
    ...
    public static function save_template($name, $config, $courseid = 0) {
        ...
        if (!$configdata) {
            throw new \moodle_exception('ajax_invalid_configuration', 'local_aiquiz_gen');
        }

        // Save as user setting.
        $DB->insert_record('local_aiquiz_gen_settings', [
            'userid' => $USER->id,
    ...
    public static function get_templates() {
        ...
        $templates = $DB->get_records_sql(
            "SELECT id, setting_name, setting_value
             FROM {local_aiquiz_gen_settings}
    ...
    public static function delete_template($templateid) {
        ...
        $template = $DB->get_record('local_aiquiz_gen_settings', ['id' => $templateid]);
        if (!$template || $template->userid != $USER->id) {
            throw new \moodle_exception('ajax_template_not_found', 'local_aiquiz_gen');
        }

        // Delete the template.
        $DB->delete_records('local_aiquiz_gen_settings', ['id' => $templateid]);
