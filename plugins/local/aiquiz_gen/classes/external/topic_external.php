...
namespace local_aiquiz_gen\external;
...
class topic_external extends external_api {
    ...
    public static function update_topic($topicid, $title) {
        ...
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $topic->requestid], '*', MUST_EXIST);
        ...
        if ($request->userid != $USER->id) {
            throw new \moodle_exception('ajax_access_denied', 'local_aiquiz_gen');
        }

        // Update the topic title.
        $DB->set_field('local_aiquiz_gen_topics', 'title', $title, ['id' => $topicid]);
    ...
    public static function merge_topics($topicid1, $topicid2) {
        ...
        // Get both topics.
        $topic1 = $DB->get_record('local_aiquiz_gen_topics', ['id' => $topicid1], '*', MUST_EXIST);
        $topic2 = $DB->get_record('local_aiquiz_gen_topics', ['id' => $topicid2], '*', MUST_EXIST);

        // Verify both topics belong to the same request.
        if ($topic1->requestid != $topic2->requestid) {
            throw new \moodle_exception('ajax_topics_same_request', 'local_aiquiz_gen');
        }

        // Get the parent request and validate context.
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $topic1->requestid], '*', MUST_EXIST);
        ...
        $DB->update_record('local_aiquiz_gen_topics', (object) [
            'id' => $topicid1,
            'title' => $newtitle,
        ...
        $DB->set_field('local_aiquiz_gen_questions', 'topicid', $topicid1, ['topicid' => $topicid2]);

        // Delete topic2.
        $DB->delete_records('local_aiquiz_gen_topics', ['id' => $topicid2]);
    ...
    public static function delete_topic($topicid) {
        ...
        $DB->delete_records('local_aiquiz_gen_questions', ['topicid' => $topicid]);
        $DB->delete_records('local_aiquiz_gen_topics', ['id' => $topicid]);
