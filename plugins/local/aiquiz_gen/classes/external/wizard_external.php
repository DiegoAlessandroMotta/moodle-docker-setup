...
class wizard_external extends external_api {
    ...
    public static function get_progress(int $requestid): array {
        ...
        $questions = $DB->get_records_sql(
            "SELECT q.id, q.questiontype, q.difficulty, q.blooms_level, q.status,
             ...
             LEFT JOIN {local_aiquiz_gen_topics} t ON t.id = q.topicid
        ...
        $topics = $DB->get_records_sql(
            "SELECT t.id, t.title, t.num_questions as target,
                    (SELECT COUNT(*) FROM {local_aiquiz_gen_questions} q WHERE q.topicid = t.id) as generated
             FROM {local_aiquiz_gen_topics} t
     ...
     *
     * Upserts into local_aiquiz_gen_wizstate. If a record already
    ...
    public static function save_wizard_state(int $courseid, int $step, int $requestid, string $state): array {
        ...
        $existing = $DB->get_record('local_aiquiz_gen_wizstate', [
            'userid' => $USER->id,
            'courseid' => $courseid,
        ]);

        if ($existing) {
            $DB->update_record('local_aiquiz_gen_wizstate', [
                'id' => $existing->id,
                'current_step' => $step,
            ...
            $DB->insert_record('local_aiquiz_gen_wizstate', [
                'userid' => $USER->id,
                'courseid' => $courseid,
                'current_step' => $step,
                'state_data' => json_encode($statedata),
    ...
    public static function get_wizard_state(int $courseid): array {
        ...
        $state = $DB->get_record('local_aiquiz_gen_wizstate', [
            'userid' => $USER->id,
            'courseid' => $courseid,
        ]);
    ...
    public static function clear_wizard_state(int $courseid): array {
        ...
        $DB->delete_records('local_aiquiz_gen_wizstate', [
            'userid' => $USER->id,
            'courseid' => $courseid,
        ]);
