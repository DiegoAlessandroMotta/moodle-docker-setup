...
/**
 * Gateway client for commercial AI Quiz Gen AI routing.
 *
 * This is a thin HTTP client that communicates with the AI Quiz Gen AI gateway.
...
/**
 * Thin gateway client for commercial AI Quiz Gen AI routing.
 */
class gateway_client {
    ...
    public static function get_gateway_key(): string {
        return trim((string)get_config('local_aiquiz_gen', 'gatewaykey'));
    }
    ...
    private static function call_gateway(string $operation, array $payload, string $quality): array {
        ...
        if ($jsonbody === false) {
            ...
            debugging("AI gateway json_encode FAILED: {$jsonerror} | operation={$operation}" .
        ...
        debugging("AI gateway sending: operation={$operation}, json_len=" . strlen($jsonbody) .
            ", topic_title=" . ($payload['topic_title'] ?? 'N/A'), DEBUG_DEVELOPER);
