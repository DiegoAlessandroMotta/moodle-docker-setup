...
class wizard_page implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        if ($this->currentstep === 'progress') {
            $stepclass = 'aiquiz-step-progress';
        } else if (is_numeric($this->currentstep)) {
            $stepclass = 'aiquiz-step-' . (int)$this->currentstep;
        } else {
            $stepclass = 'aiquiz-step-1';
        }
        ...
        return [
            'stepclass' => $stepclass,
            'wizard_title' => get_string('wizard_title', 'local_aiquiz_gen'),
            'wizard_subtitle' => get_string('wizard_subtitle', 'local_aiquiz_gen'),
            'steps' => $steps,
