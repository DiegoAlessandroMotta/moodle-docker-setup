...
class question_generator {
    ...
    public static function generate_for_topic(int $topicid, int $requestid, array $config): array {
        ...
        // Get topic details.
        $topic = $DB->get_record('local_aiquiz_gen_topics', ['id' => $topicid], '*', MUST_EXIST);
        ...
        if (!isset(self::$contentcache[$cachekey])) {
            ...
            \local_aiquiz_gen\debug_logger::debug('Content extracted for request', [
                'request_id' => $requestid,
        ...
        for ($batch = 0; $batch < $batches; $batch++) {
            ...
            for ($retry = 0; $retry <= $maxretries && !$batchsuccess; $retry++) {
                try {
                    ...
                    debugging("AI generate batch {$batch}: requested {$batchcount}, AI returned " .
                    ...
                    foreach ($result['questions'] as $qi => $questionobj) {
                        ...
                        try {
                            ...
                            debugging("AI save_question FAILED for question {$qi} in batch {$batch}: " .
                    ...
                    debugging("AI batch {$batch} EXCEPTION (retry {$retry}/{$maxretries}), topic_id={$topicid}: " .
                        $e->getMessage(), DEBUG_DEVELOPER);
                    if ($retry >= $maxretries) {
                        // Final retry failed - log error and continue.
                        debugging("AI batch {$batch} GAVE UP after {$maxretries} retries for topic {$topicid}. " .
        ...
        debugging("AI generate_for_topic DONE: requested={$numquestions}, saved=" . count($questions) .
            ", topic_id={$topicid}, types=" . json_encode($questiontypes), DEBUG_DEVELOPER);

        if (count($questions) < $numquestions) {
            debugging("AI WARNING: Only " . count($questions) . " of {$numquestions} questions were saved for topic {$topicid}." .
    ...
    private static function generate_question_batch(\stdClass $topic, array $types, array $config): array {
        ...
        debugging("AI generate_question_batch: topic_id=" . ($topic->id ?? '?') .
        ...
        \local_aiquiz_gen\debug_logger::debug('About to call gateway for question generation', [
            'topic_title' => $topic->title,
    ...
    // All AI prompts are now server-side on the AI Quiz Gen AI Gateway.
    ...
    private static function save_question(\stdClass $question): \stdClass {
        ...
        $validationenabled = get_config('local_aiquiz_gen', 'enable_question_validation') !== '0';
        if ($validationenabled && class_exists('\\local_aiquiz_gen\\question_validator')) {
            $validation = \local_aiquiz_gen\question_validator::validate_question(
                $question,
                $question->answers ?? []
            );
        ...
        try {
            $questionid = $DB->insert_record('local_aiquiz_gen_questions', $record);
        } catch (\Exception $e) {
            debugging('AI insert question FAILED: ' . $e->getMessage() .
        ...
        if (!empty($question->answers)) {
            ...
            foreach ($question->answers as $ai => $answer) {
                ...
                try {
                    $DB->insert_record('local_aiquiz_gen_answers', $answerrecord);
                } catch (\Exception $e) {
                    debugging('AI insert answer FAILED for question ' . $questionid . ', answer ' . $ai .
                        ': ' . $e->getMessage() .
    ...
    private static function validate_config(array $config): void {
        if (empty($config['question_types'])) {
            throw new \moodle_exception('error:noquestiontypes', 'local_aiquiz_gen');
        }

        foreach ($config['question_types'] as $type) {
            if (!in_array($type, self::QUESTION_TYPES)) {
                throw new \moodle_exception('error:invalidquestiontype', 'local_aiquiz_gen', '', $type);
            }
        }

        if (isset($config['difficulty']) && !in_array($config['difficulty'], self::DIFFICULTY_LEVELS)) {
            throw new \moodle_exception('error:invaliddifficulty', 'local_aiquiz_gen');
        }
    }
    ...
    private static function get_full_content_for_request(\stdClass $request): string {
        ...
        $files = $fs->get_area_files($context->id, 'local_aiquiz_gen', 'content', $request->id, 'filename', false);
        ...
        if (!empty($request->content_sources)) {
            ...
            foreach ($sources as $source) {
                if (strpos($source, 'course_activities:') === 0) {
                    ...
                    try {
                        $scanresult = \local_aiquiz_gen\course_scanner::scan_entire_course($request->courseid);
                        if (!empty(trim($scanresult['text']))) {
                            $fullcontent .= "\n\n" . $scanresult['text'];
                        }
                    } catch (\Exception $e) {
                        // Fallback to stored topic data if re-extraction fails.
                        $topics = $DB->get_records('local_aiquiz_gen_topics', [
        ...
        $rs = $DB->get_recordset('local_aiquiz_gen_urlcont', ['requestid' => $request->id]);
        foreach ($rs as $url) {
