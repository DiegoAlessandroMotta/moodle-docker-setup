...
class quiz_deployer {
    ...
    public static function deploy_to_question_bank(
        ...
        foreach ($questionids as $questionid) {
            try {
                ...
                if (!isset($allgenquestions[$questionid])) {
                    throw new \moodle_exception('questionnotfound', 'local_aiquiz_gen');
                }
                ...
                if (!empty($genquestion->moodle_questionid)) {
                    ...
                    if ($existingmq) {
                        ...
                        $DB->set_field('local_aiquiz_gen_questions', 'status', 'deployed', ['id' => $questionid]);
                        $questionnumber++;
                        continue;
                    }
        ...
        if (empty($deployedids)) {
            ...
            throw new \moodle_exception('error:deployment', 'local_aiquiz_gen', '', $errormsg);
        }
    ...
    public static function create_quiz(array $questionids, int $courseid, string $quizname, array $settings = []): int {
        ...
        if (empty($moodlequestionids)) {
            throw new \moodle_exception('error:deployment', 'local_aiquiz_gen', '', 'No questions deployed');
        }
    ...
    private static function tag_question(int $questionid, \stdClass $genquestion, \stdClass $category): void {
        ...
        if (!empty($genquestion->topicid)) {
            $topic = $DB->get_record('local_aiquiz_gen_topics', ['id' => $genquestion->topicid], 'title');
            if ($topic) {
    ...
    private static function add_multichoice_data(int $questionid, \stdClass $genquestion): void {
        ...
        // Add answers.
        $rs = $DB->get_recordset('local_aiquiz_gen_answers', ['questionid' => $genquestion->id], 'sortorder ASC');
    ...
    private static function add_essay_data(int $questionid, \stdClass $genquestion): void {
        ...
        $answers = $DB->get_records('local_aiquiz_gen_answers', ['questionid' => $genquestion->id], 'sortorder ASC');
        if (!empty($answers)) {
