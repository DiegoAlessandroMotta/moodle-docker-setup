...
class rate_limiter {
    ...
    public static function check_rate_limit($userid, $courseid = 0) {
        ...
        $limitperhour = get_config('local_aiquiz_gen', 'rate_limit_per_hour')
            ?: self::DEFAULT_LIMIT_PER_HOUR;
        $limitperday = get_config('local_aiquiz_gen', 'rate_limit_per_day')
            ?: self::DEFAULT_LIMIT_PER_DAY;
        $sitelimitperhour = get_config('local_aiquiz_gen', 'site_rate_limit_per_hour')
    ...
    public static function is_rate_limiting_enabled() {
        return (bool) get_config('local_aiquiz_gen', 'enable_rate_limiting');
    }
    ...
    public static function record_violation($userid, $limittype, $details = []) {
        ...
        try {
            $DB->insert_record('local_aiquiz_gen_ratelog', $record);
        } catch (\Exception $e) {
    ...
    public static function is_user_exempt($userid) {
        ...
        $exemptions = get_config('local_aiquiz_gen', 'rate_limit_exempt_users');
        if ($exemptions) {
