...
class cleanup_old_requests extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('task:cleanupoldrequest', 'local_aiquiz_gen');
    }
    ...
    public function execute() {
        ...
        $cleanupdays = get_config('local_aiquiz_gen', 'cleanup_days');
    ...
    private function delete_request(int $requestid): void {
        ...
        try {
            ...
            $qrs = $DB->get_recordset('local_aiquiz_gen_questions', ['requestid' => $requestid], '', 'id');
            foreach ($qrs as $question) {
                $DB->delete_records('local_aiquiz_gen_answers', ['questionid' => $question->id]);
            }
            $qrs->close();
            ...
            // Delete topics.
            $DB->delete_records('local_aiquiz_gen_topics', ['requestid' => $requestid]);

            // Delete logs.
            $DB->delete_records('local_aiquiz_gen_logs', ['requestid' => $requestid]);

            // Delete request.
            $DB->delete_records('local_aiquiz_gen_requests', ['id' => $requestid]);
