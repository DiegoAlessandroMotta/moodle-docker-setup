...
namespace local_aiquiz_gen\task;
...
class generate_questions_adhoc extends \core\task\adhoc_task {
    ...
    public function execute() {
        ...
        // Log task start.
        \local_aiquiz_gen\debug_logger::info('Adhoc task started', [
        ...
        try {
            ...
            // Get selected topics.
            $topics = \local_aiquiz_gen\topic_analyzer::get_selected_topics($requestid);

            \local_aiquiz_gen\debug_logger::debug('Topics retrieved', [
                'topics_count' => count($topics),
                'topic_ids' => array_keys($topics),
            ], $requestid);

            if (empty($topics)) {
                \local_aiquiz_gen\debug_logger::error('No topics selected for generation', [
                    'request_id' => $requestid,
                ], $requestid);
                throw new \moodle_exception('error:notopicsselected', 'local_aiquiz_gen');
            }
            ...
            foreach ($topics as $topic) {
                ...
                $questions = \local_aiquiz_gen\question_generator::generate_for_topic(
                    $topic->id,
                    $requestid,
                    $topicconfig
                );
                ...
                debugging("AI adhoc topic '{$topic->title}' (id={$topic->id}): requested={$topic->num_questions}, " .
            ...
            debugging(
                "AI adhoc SUMMARY: total_requested={$totalquestionsrequested}, total_generated={$totalquestionsgenerated}, " .
                "topics=" . count($topics),
                DEBUG_DEVELOPER
            );
            if ($totalquestionsgenerated < $totalquestionsrequested) {
                debugging("AI adhoc WARNING: Missing " . ($totalquestionsrequested - $totalquestionsgenerated) .
                    " questions. Check per-topic and save_question logs above.", DEBUG_DEVELOPER);
            }
            $DB->set_field('local_aiquiz_gen_requests', 'total_questions', $totalquestionsgenerated, ['id' => $requestid]);
            ...
            $allquestions = $DB->get_records('local_aiquiz_gen_questions', ['requestid' => $requestid], 'id ASC');
            $questiontexts = [];
            $questionids = [];
            ...
            if (count($questiontexts) > 1) {
                $duplicates = \local_aiquiz_gen\question_validator::check_for_duplicates($questiontexts);
                if (!empty($duplicates)) {
            ...
            if (!empty($idstoremove)) {
                foreach (array_keys($idstoremove) as $removeid) {
                    $DB->delete_records('local_aiquiz_gen_answers', ['questionid' => $removeid]);
                    $DB->delete_records('local_aiquiz_gen_questions', ['id' => $removeid]);
                ...
                if ($shortfall > 0 && !empty($topics)) {
                    ...
                    try {
                        $replacequestions = \local_aiquiz_gen\question_generator::generate_for_topic(
                            $replacetopic->id,
                            $requestid,
                            $replaceconfig
                        );
                        ...
                        \local_aiquiz_gen\debug_logger::debug(
            ...
            \local_aiquiz_gen\api::update_request_status($requestid, 'completed');
        } catch (\Exception $e) {
            // Log the exception with full details.
            \local_aiquiz_gen\debug_logger::exception($e, 'generate_questions_adhoc', $requestid);
            ...
            // Log task failure.
            \local_aiquiz_gen\debug_logger::error('Adhoc task failed', [
                'task' => 'generate_questions_adhoc',
                'request_id' => $requestid,
                'error' => $e->getMessage(),
            ], $requestid);
    ...
    private static function update_progress(int $requestid, string $status, float $progress, string $message): void {
        ...
        $DB->update_record('local_aiquiz_gen_requests', $record);
    }
}
