...
class process_generation_queue extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('task:processgenerationqueue', 'local_aiquiz_gen');
    }
    ...
    public function execute() {
        ...
        $requests = $DB->get_records('local_aiquiz_gen_requests', ['status' => 'pending'], 'timecreated ASC', '*', 0, 5);
    ...
    private function process_request(\stdClass $request): void {
        ...
        \local_aiquiz_gen\api::update_request_status($request->id, 'processing');
        ...
        foreach ($topics as $topic) {
            ...
            foreach ($questionsbydiff as $difficulty => $count) {
                ...
                // Update progress.
                $DB->set_field('local_aiquiz_gen_requests', 'questions_generated', $totalgenerated, ['id' => $request->id]);
            }
        }

        // Mark as completed using centralized status tracking.
        \local_aiquiz_gen\api::update_request_status($request->id, 'completed');
    ...
    private function mark_request_failed(int $requestid, string $error): void {
        ...
        \local_aiquiz_gen\api::update_request_status($requestid, 'failed', $error);
    ...
    private function send_completion_notification(\stdClass $request): void {
        ...
        $message->component = 'local_aiquiz_gen';
        $message->name = 'generation_complete';
        $message->userfrom = \core_user::get_noreply_user();
        $message->userto = $user;
        $message->subject = get_string('notification:generation_complete_subject', 'local_aiquiz_gen');
        $message->fullmessage = get_string('notification:generation_complete_body', 'local_aiquiz_gen', [
            'coursename' => $course->fullname,
        ]);
        $message->fullmessageformat = FORMAT_PLAIN;
        $message->fullmessagehtml = '';
        $message->smallmessage = get_string('notification:generation_complete_subject', 'local_aiquiz_gen');
    ...
    private function send_failure_notification(\stdClass $request, string $error): void {
        ...
        $message->userfrom = \core_user::get_noreply_user();
        $message->userto = $user;
        $message->subject = get_string('notification:generation_failed_subject', 'local_aiquiz_gen');
        $message->fullmessage = get_string('notification:generation_failed_body', 'local_aiquiz_gen', [
            'coursename' => $course->fullname,
            'error' => $error,
        ]);
        $message->fullmessageformat = FORMAT_PLAIN;
        $message->fullmessagehtml = '';
        $message->smallmessage = get_string('notification:generation_failed_subject', 'local_aiquiz_gen');
        $message->notification = 1;
