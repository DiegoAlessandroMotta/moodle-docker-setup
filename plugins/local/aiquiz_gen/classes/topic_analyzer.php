...
class topic_analyzer {
    ...
    public static function analyze_content(string $content, int $requestid): array {
        ...
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $requestid], 'courseid');
    ...
    // NOTE: AI prompts are proprietary and located on the AI Quiz Gen AI Gateway server.
    ...
    private static function save_topics(array $topics, int $requestid): array {
        ...
        foreach ($topics as $maintopic) {
            ...
            try {
                $topicid = $DB->insert_record('local_aiquiz_gen_topics', $topicrecord);
            } catch (\Exception $e) {
                debugging('AI save_topics main topic insert FAILED: ' . $e->getMessage() .
            ...
            if (!empty($maintopic['subtopics'])) {
                foreach ($maintopic['subtopics'] as $subtopic) {
                    ...
                    try {
                        $subtopicid = $DB->insert_record('local_aiquiz_gen_topics', $subtopicrecord);
                    } catch (\Exception $e) {
                        debugging('AI save_topics subtopic insert FAILED: ' . $e->getMessage() .
    ...
    public static function update_topic_selection(int $topicid, bool $selected, int $numquestions = 0): bool {
        ...
        return $DB->update_record('local_aiquiz_gen_topics', $record);
    }
    ...
    public static function get_selected_topics(int $requestid): array {
        ...
        return $DB->get_records('local_aiquiz_gen_topics', [
            'requestid' => $requestid,
            'selected' => 1,
        ], 'level ASC, id ASC');
    }
    ...
    private static function clone_topics_from_cache(array $cachedtopics, int $requestid): array {
        ...
        foreach ($cachedtopics as $topic) {
            ...
            $newid = $DB->insert_record('local_aiquiz_gen_topics', $newtopic);
