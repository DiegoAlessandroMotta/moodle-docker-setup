...
if ($action === 'clearfile' && confirm_sesskey()) {
    ...
    $message = get_string('debuglogs_action_clearfile_success', 'local_aiquiz_gen');
    redirect(
        new moodle_url('/local/aiquiz_gen/debug_logs.php', ['tab' => 'file']),
        $message,
        null,
...
if ($action === 'logsysteminfo' && confirm_sesskey()) {
    ...
    $message = get_string('debuglogs_action_logsysteminfo_success', 'local_aiquiz_gen');
    redirect(
        new moodle_url('/local/aiquiz_gen/debug_logs.php', ['tab' => $tab]),
        $message,
        null,
        \core\output\notification::NOTIFY_SUCCESS
    );
...
if ($action === 'testlog' && confirm_sesskey()) {
    ...
    $message = get_string('debuglogs_action_testlog_success', 'local_aiquiz_gen');
    redirect(
        new moodle_url('/local/aiquiz_gen/debug_logs.php', ['tab' => $tab]),
        $message,
        null,
...
$PAGE->set_url('/local/aiquiz_gen/debug_logs.php', ['tab' => $tab]);
$pagetitle = get_string('pluginname', 'local_aiquiz_gen') . ' - ' .
    get_string('debuglogs_title', 'local_aiquiz_gen');
...
$PAGE->requires->js_call_amd('local_aiquiz_gen/debuglogs', 'init');

echo $OUTPUT->header();
$heading = get_string('debuglogs_pagetitle', 'local_aiquiz_gen');
echo $OUTPUT->heading($heading);

// Render the debug logs page using templates and the Output API.
$page = new \local_aiquiz_gen\output\debug_logs_page($tab, $requestid, $level, $limit);
echo $OUTPUT->render_from_template('local_aiquiz_gen/debug_logs', $page->export_for_template($OUTPUT));
