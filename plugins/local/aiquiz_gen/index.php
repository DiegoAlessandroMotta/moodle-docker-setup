...
if ($action === 'wizard') {
    redirect(new moodle_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid]));
}

// Page setup.
$PAGE->set_url(new moodle_url('/local/aiquiz_gen/index.php', ['courseid' => $courseid]));
$PAGE->set_context($context);
$PAGE->set_course($course);
$PAGE->set_pagelayout('standard');
$PAGE->set_title(get_string('pluginname', 'local_aiquiz_gen') . ' - ' . get_string('dashboard', 'local_aiquiz_gen'));
...
$totalquizzes = $DB->count_records_sql(
    "SELECT COUNT(DISTINCT id) FROM {local_aiquiz_gen_requests}
...
$totalquestions = $DB->count_records('local_aiquiz_gen_questions', ['userid' => $userid]);
...
$pendingquestions = $DB->count_records('local_aiquiz_gen_questions', [
    'userid' => $userid,
    'status' => 'pending',
]);
...
$recentrequests = $DB->get_records_sql(
    "SELECT r.id, r.courseid, r.status, r.total_questions, r.questions_generated,
     ...
     FROM {local_aiquiz_gen_requests} r
...
$coursequestions = $DB->count_records('local_aiquiz_gen_questions', [
    'userid' => $userid,
    'courseid' => $courseid,
]);
...
$PAGE->requires->js_call_amd('local_aiquiz_gen/dashboard', 'init', [[
    'courseid' => $courseid,
    'sesskey' => sesskey(),
    'stats' => [
...
$page = new \local_aiquiz_gen\output\index_page(
    $courseid,
    $totalquizzes,
    $coursequizzes,
    $totalquestions,
...
echo $OUTPUT->render_from_template('local_aiquiz_gen/index', $page->export_for_template($OUTPUT));
