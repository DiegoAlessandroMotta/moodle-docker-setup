...
use local_aiquiz_gen\wizard_helper;
...
$PAGE->set_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid, 'step' => $step]);
$PAGE->set_context($context);
$PAGE->set_title(get_string('wizard_title', 'local_aiquiz_gen'));
$PAGE->set_heading($course->fullname);
$PAGE->set_pagelayout('standard');

// Add step-specific body class so we can target footer behavior per step.
$PAGE->add_body_class('aiquiz-step-' . (is_numeric($step) ? (int)$step : $step));
...
if (!empty($dependencyerrors)) {
    ...
    if ($criticalerror) {
        echo $OUTPUT->notification(
            get_string('error:noaiprovider', 'local_aiquiz_gen'),
    ...
    echo html_writer::div(
        html_writer::link(
            ...
            '<i class="fa fa-arrow-left aiquiz-icon-secondary"></i> ' . get_string('back'),
...
if ($action === 'upload_content' && confirm_sesskey()) {
    local_aiquiz_gen_handle_content_upload($courseid, $context);
}

if ($action === 'create_request' && confirm_sesskey()) {
    local_aiquiz_gen_handle_create_request($courseid);
}

if ($action === 'save_topic_selection' && confirm_sesskey()) {
    local_aiquiz_gen_handle_save_topic_selection($requestid);
}

if ($action === 'save_question_distribution' && confirm_sesskey()) {
    local_aiquiz_gen_handle_save_question_distribution($requestid);
}

if ($action === 'generate_questions' && confirm_sesskey()) {
    local_aiquiz_gen_handle_generate_questions($requestid);
}

if ($action === 'deploy_questions' && confirm_sesskey()) {
    local_aiquiz_gen_handle_deploy_questions($requestid, $courseid);
}

// Handle individual question actions (Step 4).
if ($action === 'approve_question' && confirm_sesskey()) {
    $questionid = required_param('questionid', PARAM_INT);
    $DB->set_field('local_aiquiz_gen_questions', 'status', 'approved', ['id' => $questionid]);
    redirect(new moodle_url('/local/aiquiz_gen/wizard.php', [
        'courseid' => $courseid,
        'requestid' => $requestid,
        'step' => 4,
    ]));
...
if ($action === 'approve_all_questions' && confirm_sesskey()) {
    ...
    redirect(
        new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid,
            'requestid' => $requestid,
            'step' => 4,
        ]),
        get_string('bulk_approved', 'local_aiquiz_gen', $approvedcount),
        null,
...
if ($action === 'reject_question' && confirm_sesskey()) {
    $questionid = required_param('questionid', PARAM_INT);
    $DB->set_field('local_aiquiz_gen_questions', 'status', 'rejected', ['id' => $questionid]);
    redirect(new moodle_url('/local/aiquiz_gen/wizard.php', [
        'courseid' => $courseid,
        'requestid' => $requestid,
        'step' => 4,
...
if ($action === 'regenerate_question' && confirm_sesskey()) {
    ...
    try {
        ...
        $newquestion = \local_aiquiz_gen\api::regenerate_question($questionid);
        ...
        redirect(
            ...
            get_string('wizard_question_regenerated', 'local_aiquiz_gen'),
            null,
            \core\output\notification::NOTIFY_SUCCESS
        );
    } catch (\Exception $e) {
        ...
        if (empty($requestid) || $requestid == 0) {
            $question = $DB->get_record('local_aiquiz_gen_questions', ['id' => $questionid], 'requestid');
            if ($question) {
...
if (!empty($bulkaction) && confirm_sesskey()) {
    local_aiquiz_gen_handle_bulk_action($bulkaction, $requestid);
}
...
if ($step === '3.5') {
    $jsconfig['refreshUrl'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
        'courseid' => $courseid,
        'requestid' => $requestid,
        'step' => '3.5',
    ]))->out(false);
...
if ($step == 1) {
    $jsconfig['strings'] = [
        'choose_files' => get_string('choose_files', 'local_aiquiz_gen'),
    ];
    $jsconfig['maxActivities'] = (int) (get_config('local_aiquiz_gen', 'max_activities') ?: 20);
}
$PAGE->requires->js_call_amd('local_aiquiz_gen/wizard', 'init', [$jsconfig]);
...
switch ($step) {
    case '1':
    case 1:
        $stephtml = local_aiquiz_gen_render_step1($courseid, $requestid);
        break;
    case '2':
    case 2:
        $stephtml = local_aiquiz_gen_render_step2($courseid, $requestid);
        break;
    case '3':
    case 3:
        $stephtml = local_aiquiz_gen_render_step3($courseid, $requestid);
        break;
    case 'progress':
        $stephtml = local_aiquiz_gen_render_step3_5($courseid, $requestid);
        break;
    case '4':
    case 4:
        $stephtml = local_aiquiz_gen_render_step4($courseid, $requestid);
        break;
    case '5':
    case 5:
        $stephtml = local_aiquiz_gen_render_step5($courseid, $requestid);
        break;
    default:
        $stephtml = local_aiquiz_gen_render_step1($courseid, $requestid);
}
...
$wizardpage = new \local_aiquiz_gen\output\wizard_page($step, $stephtml);
echo $OUTPUT->render_from_template('local_aiquiz_gen/wizard', $wizardpage->export_for_template($OUTPUT));
 ...
 * @return void
 */
function local_aiquiz_gen_handle_content_upload(int $courseid, context $context) {
    global $DB, $USER, $CFG;

    // Check rate limit BEFORE processing uploads to avoid wasting resources.
    if (
        \local_aiquiz_gen\rate_limiter::is_rate_limiting_enabled() &&
        !\local_aiquiz_gen\rate_limiter::is_user_exempt($USER->id)
    ) {
        $ratelimitcheck = \local_aiquiz_gen\rate_limiter::check_rate_limit($USER->id, $courseid);

        if (!$ratelimitcheck['allowed']) {
            // Record violation.
            \local_aiquiz_gen\rate_limiter::record_violation(
                $USER->id,
            ...
            redirect(
                new moodle_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid, 'step' => 1]),
                get_string('error:rate_limit_exceeded', 'local_aiquiz_gen', $ratelimitcheck['reason']),
                null,
                \core\output\notification::NOTIFY_ERROR
            );
            return;
    ...
    $uploadedfiledata = local_aiquiz_gen_get_uploaded_files('contentfiles');

    // Validate file sizes before processing.
    if ($uploadedfiledata && !empty($uploadedfiledata['name'][0])) {
        $maxfilesize = get_config('local_aiquiz_gen', 'max_file_size_mb') ?: 50;
    ...
    if (!$hasmanualtext && !$hasfiles && !$hasurls && !$hasactivities && !$hasbulkscan) {
        redirect(
            new moodle_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid, 'step' => 1]),
            get_string('error:nocontent', 'local_aiquiz_gen'),
            null,
            \core\output\notification::NOTIFY_ERROR
        );
        return;
    ...
    $maxextractionmb = (int)(get_config('local_aiquiz_gen', 'max_extraction_size_mb') ?: 100);
    if ($hasactivities || $hasbulkscan) {
        ...
        if ($totalsizemb > $maxextractionmb) {
            redirect(
                new moodle_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid, 'step' => 1]),
                get_string('error:content_too_large', 'local_aiquiz_gen', (object)[
                    'size' => $totalsizemb,
                    'max' => $maxextractionmb,
                ]),
                null,
    ...
    $deduplicationenabled = get_config('local_aiquiz_gen', 'enable_content_deduplication') !== '0';
    $existingrequest = null;

    if ($deduplicationenabled) {
        // Check for duplicate content in this course from the last 30 days.
        $existingrequest = $DB->get_record_sql(
            "SELECT * FROM {local_aiquiz_gen_requests}
    ...
    if ($existingrequest && $existingrequest->status === 'completed') {
        ...
        $existingtopics = $DB->get_records('local_aiquiz_gen_topics', ['requestid' => $existingrequest->id]);
        ...
        if (!empty($existingtopics)) {
            ...
            $requestid = $DB->insert_record('local_aiquiz_gen_requests', $record);
            ...
            foreach ($existingtopics as $topic) {
                ...
                $cleanedtitle = \local_aiquiz_gen\topic_analyzer::clean_topic_title_public($topic->title);
            ...
            $DB->insert_records('local_aiquiz_gen_topics', $inserttopics);
    ...
    \local_aiquiz_gen\api::update_request_status($requestid, 'analyzing');
    ...
    if ($hasfiles) {
        ...
        foreach ($uploadedfiledata['name'] as $key => $filename) {
            ...
            if ($fileerror != UPLOAD_ERR_OK) {
                $errormsg = get_string('wizard_upload_err_unknown', 'local_aiquiz_gen');
                switch ($fileerror) {
                    case UPLOAD_ERR_INI_SIZE:
                        $errormsg = get_string('wizard_upload_err_ini_size', 'local_aiquiz_gen');
                        break;
                    case UPLOAD_ERR_FORM_SIZE:
                        $errormsg = get_string('wizard_upload_err_form_size', 'local_aiquiz_gen');
                        break;
                    case UPLOAD_ERR_PARTIAL:
                        $errormsg = get_string('wizard_upload_err_partial', 'local_aiquiz_gen');
                        break;
                    case UPLOAD_ERR_NO_FILE:
                        $errormsg = get_string('wizard_upload_err_no_file', 'local_aiquiz_gen');
                        break;
                    case UPLOAD_ERR_NO_TMP_DIR:
                        $errormsg = get_string('wizard_upload_err_no_tmp_dir', 'local_aiquiz_gen');
                        break;
                    case UPLOAD_ERR_CANT_WRITE:
                        $errormsg = get_string('wizard_upload_err_cant_write', 'local_aiquiz_gen');
                        break;
                    case UPLOAD_ERR_EXTENSION:
                        $errormsg = get_string('wizard_upload_err_extension', 'local_aiquiz_gen');
                        break;
                }
                redirect(
                    new moodle_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid, 'step' => 1]),
                    get_string('error:fileupload', 'local_aiquiz_gen', ['filename' => $filename, 'error' => $errormsg]),
                    null,
                    \core\output\notification::NOTIFY_ERROR
                );
                return;
            ...
            if (!file_exists($tmpfile)) {
                redirect(
                    new moodle_url('/local/aiquiz_gen/wizard.php', ['courseid' => $courseid, 'step' => 1]),
                    get_string('error:fileupload', 'local_aiquiz_gen', [
                        'filename' => $filename,
                        'error' => get_string('wizard_tmp_file_not_found', 'local_aiquiz_gen'),
                    ]),
                    null,
                    \core\output\notification::NOTIFY_ERROR
                );
    ...
    if ($hasurls) {
        require_once($CFG->dirroot . '/local/aiquiz_gen/classes/content_extractor.php');

        foreach ($urls as $url) {
            try {
                $urlcontent = \local_aiquiz_gen\content_extractor::extract_from_url($url);
                ...
                $DB->insert_record('local_aiquiz_gen_urlcont', $urlrecord);
            } catch (Exception $e) {
 ...
 * @return void
 */
function local_aiquiz_gen_handle_create_request(int $courseid) {
    global $DB, $USER;
 ...
 * @return void
 */
function local_aiquiz_gen_handle_save_question_distribution(int $requestid) {
    global $DB;
    ...
    foreach ($topicquestions as $topicid => $numqs) {
        ...
        $DB->set_field('local_aiquiz_gen_topics', 'num_questions', $numqs, ['id' => $topicid]);
    }

    // Update total question count on request.
    $totalquestions = array_sum($topicquestions);
    $DB->set_field('local_aiquiz_gen_requests', 'total_questions', $totalquestions, ['id' => $requestid]);
 ...
 * @return void
 */
function local_aiquiz_gen_handle_save_topic_selection(int $requestid) {
    global $DB;
    ...
    $DB->set_field('local_aiquiz_gen_topics', 'selected', 0, ['requestid' => $requestid]);
    ...
    if (!empty($selectedtopics)) {
        ...
        $DB->set_field_select('local_aiquiz_gen_topics', 'selected', 1, "id " . $insql, $inparams);
        $DB->set_field_select('local_aiquiz_gen_topics', 'num_questions', 5, "id " . $insql, $inparams);
    }
 ...
 */
function local_aiquiz_gen_handle_generate_questions(int $requestid) {
    global $DB;

    $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $requestid], '*', MUST_EXIST);
    ...
    if ($totalquestions < 1 || $totalquestions > 100) {
        throw new \moodle_exception('wizard_invalid_total_questions', 'local_aiquiz_gen');
    }
    ...
    if (!empty($questiontypedist) && $typecount != $totalquestions) {
        ...
        throw new \moodle_exception('wizard_qtype_count_mismatch', 'local_aiquiz_gen', '', $mismatchinfo);
    }
    ...
    $processingmode = get_config('local_aiquiz_gen', 'default_quality_mode') ?? 'balanced';
    ...
    $existingquestions = $DB->get_records('local_aiquiz_gen_questions', ['requestid' => $requestid], '', 'id');
    if (!empty($existingquestions)) {
        foreach ($existingquestions as $eq) {
            $DB->delete_records('local_aiquiz_gen_answers', ['questionid' => $eq->id]);
        }
        $DB->delete_records('local_aiquiz_gen_questions', ['requestid' => $requestid]);
    }

    // CRITICAL FIX: Distribute total questions across ALL selected topics.
    $selectedtopics = $DB->get_records('local_aiquiz_gen_topics', ['requestid' => $requestid, 'selected' => 1]);
    ...
    if ($topiccount > 0) {
        ...
        foreach ($selectedtopics as $topic) {
            ...
            $DB->set_field('local_aiquiz_gen_topics', 'num_questions', $topicquestions, ['id' => $topic->id]);
    ...
    try {
        ...
        $questionsgenerated = $DB->count_records('local_aiquiz_gen_questions', ['requestid' => $requestid]);
        ...
        \local_aiquiz_gen\error_handler::handle_exception(
            $e,
            $requestid,
            'question_generation',
            \local_aiquiz_gen\error_handler::SEVERITY_ERROR
        );
        \local_aiquiz_gen\api::update_request_status($requestid, 'failed', $e->getMessage());
        $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $requestid], '*', MUST_EXIST);
 ...
 * @return void
 */
function local_aiquiz_gen_handle_bulk_action(string $action, int $requestid) {
    global $DB;
    ...
    if (empty($questionids)) {
        redirect(
            new moodle_url('/local/aiquiz_gen/wizard.php', [
                'courseid' => $DB->get_field('local_aiquiz_gen_requests', 'courseid', ['id' => $requestid]),
                'requestid' => $requestid,
                'step' => 4,
            ]),
            get_string('error:noquestionsselected', 'local_aiquiz_gen'),
            null,
            \core\output\notification::NOTIFY_ERROR
        );
        return;
    ...
    foreach ($questionids as $qid) {
        ...
        switch ($action) {
            case 'approve':
                ...
                $DB->update_record('local_aiquiz_gen_questions', $question);
                $count++;
                break;
            ...
            case 'delete':
                ...
                $DB->delete_records('local_aiquiz_gen_answers', ['questionid' => $qid]);
                // Delete question.
                $DB->delete_records('local_aiquiz_gen_questions', ['id' => $qid]);
                $count++;
                break;
        }
    }
    ...
    $messages = [
        'approve' => get_string('bulk_approved', 'local_aiquiz_gen', $count),
        'reject' => get_string('bulk_rejected', 'local_aiquiz_gen', $count),
        'delete' => get_string('bulk_deleted', 'local_aiquiz_gen', $count),
    ];
    ...
    redirect(
        ...
        $messages[$action] ?? get_string('wizard_action_completed', 'local_aiquiz_gen'),
        null,
        \core\output\notification::NOTIFY_SUCCESS
    );
}
 ...
 */
function local_aiquiz_gen_auto_recover_tracking(array $questionids, int $courseid, $DB): array {
    $tracked = 0;
    $recovered = 0;
    $untracked = 0;
    ...
    $questions = $DB->get_records_list('local_aiquiz_gen_questions', 'id', $questionids);
    ...
    foreach ($questions as $q) {
        ...
        if (!empty($q->moodle_questionid)) {
            $DB->set_field('local_aiquiz_gen_questions', 'status', 'deployed', ['id' => $qid]);
            $tracked++;
            continue;
        }
        ...
        if (!empty($questiontext) && !empty($bankqids)) {
            ...
            if ($match) {
                ...
                $DB->update_record('local_aiquiz_gen_questions', $updateobj);
                $recovered++;
        ...
        $DB->set_field('local_aiquiz_gen_questions', 'status', 'approved', ['id' => $qid]);
        $untracked++;
    }

    $message = '';
    if ($recovered > 0) {
        $message .= get_string('wizard_auto_recovered', 'local_aiquiz_gen', $recovered) . ' ';
    }
    if ($untracked > 0) {
        $message .= get_string('wizard_untracked_warning', 'local_aiquiz_gen', $untracked);
    }
 ...
 * @throws moodle_exception
 */
function local_aiquiz_gen_handle_deploy_questions(int $requestid, int $courseid) {
    global $DB;
    ...
    $questions = $DB->get_records('local_aiquiz_gen_questions', [
        'requestid' => $requestid,
        'status' => 'approved',
    ], '', 'id, questiontype');
    ...
    if (empty($questionids)) {
        ...
        $allquestions = $DB->count_records('local_aiquiz_gen_questions', ['requestid' => $requestid]);
        if ($allquestions > 0) {
            ...
            redirect(
                ...
                get_string('wizard_cannot_deploy_none_approved', 'local_aiquiz_gen', $allquestions),
                null,
                \core\output\notification::NOTIFY_ERROR
            );
        } else {
            throw new \moodle_exception('error:noquestionstodeploy', 'local_aiquiz_gen');
        }
    }
    ...
    try {
        $deployer = new \local_aiquiz_gen\quiz_deployer();
        ...
        if ($deploytype === 'new_quiz') {
            ...
            $result = local_aiquiz_gen_auto_recover_tracking($questionids, $courseid, $DB);

            $successmsg = get_string('success:quizcreated', 'local_aiquiz_gen');
            ...
            if ($result['untracked'] > 0) {
                ...
                redirect(
                    new moodle_url('/mod/quiz/view.php', ['id' => $cmid]),
                    $successmsg . ' ' . get_string('wizard_all_questions_tracked', 'local_aiquiz_gen', $result['tracked']),
                    null,
                    \core\output\notification::NOTIFY_SUCCESS
                );
            }
            ...
            $deploymsg->category = $categoryname ?: get_string('wizard_default', 'local_aiquiz_gen');
            $successmsg = get_string('success:questionsdeployed', 'local_aiquiz_gen') .
                         ' ' . get_string('wizard_questions_to_category', 'local_aiquiz_gen', $deploymsg);
        ...
        redirect(
            ...
            get_string('error:deploymentfailed', 'local_aiquiz_gen') . ': ' . $fullerror,
            null,
            \core\output\notification::NOTIFY_ERROR
        );
    }
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step_indicator($currentstep): string {
    ...
    $steps = [
        1 => get_string('step1_title', 'local_aiquiz_gen'),
        2 => get_string('step2_title', 'local_aiquiz_gen'),
        3 => get_string('step3_title', 'local_aiquiz_gen'),
        4 => get_string('step4_title', 'local_aiquiz_gen'),
        5 => get_string('step5_title', 'local_aiquiz_gen'),
    ];

    $html = html_writer::start_div('aiquiz-steps mb-5');

    foreach ($steps as $stepnum => $steptitle) {
        $classes = ['aiquiz-step-item'];
        ...
        $html .= html_writer::tag('div', $stepnum, ['class' => 'aiquiz-step-marker']);
        $html .= html_writer::start_div('aiquiz-step-details');
        $html .= html_writer::tag('div', $steptitle, ['class' => 'aiquiz-step-title']);
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step1(int $courseid, int $requestid): string {
    global $OUTPUT, $DB, $PAGE;

    $formurl = new moodle_url('/local/aiquiz_gen/wizard.php', [
        'courseid' => $courseid,
    ...
    foreach ($modinfo->get_cms() as $cm) {
        if (in_array($cm->modname, $allowedmodules) && $cm->uservisible) {
            ...
            $emojimap = [
                'page' => '<i class="fa fa-file-text-o aiquiz-icon-purple"></i>',
                'book' => '<i class="fa fa-book aiquiz-icon-primary"></i>',
                'lesson' => '<i class="fa fa-graduation-cap aiquiz-icon-info"></i>',
                'resource' => '<i class="fa fa-paperclip aiquiz-icon-purple"></i>',
                'url' => '<i class="fa fa-link aiquiz-icon-info"></i>',
                'folder' => '<i class="fa fa-folder aiquiz-icon-warning"></i>',
                'scorm' => '<i class="fa fa-cube aiquiz-icon-primary"></i>',
                'forum' => '<i class="fa fa-comments aiquiz-icon-info"></i>',
            ];
            $emoji = $emojimap[$cm->modname] ?? '<i class="fa fa-file aiquiz-icon-info"></i>';
    ...
    $context = [
        'step1_title' => get_string('step1_title', 'local_aiquiz_gen'),
        'step1_description' => get_string('step1_description', 'local_aiquiz_gen'),
        'form_url' => $formurl->out(false),
        'sesskey' => sesskey(),
        'courseid' => $courseid,
        'select_content_sources' => get_string('select_content_sources', 'local_aiquiz_gen'),
        'select_content_sources_help' => get_string('select_content_sources_help', 'local_aiquiz_gen'),
        'group1_title' => get_string('wizard_add_your_own_content', 'local_aiquiz_gen'),
        'source_manual' => get_string('source_manual', 'local_aiquiz_gen'),
        'source_manual_desc' => get_string('source_manual_desc', 'local_aiquiz_gen'),
        'source_upload' => get_string('source_upload', 'local_aiquiz_gen'),
        'source_upload_desc' => get_string('source_upload_desc', 'local_aiquiz_gen'),
        'source_url_title' => get_string('wizard_extract_from_url', 'local_aiquiz_gen'),
        'source_url_desc' => get_string('wizard_fetch_content_from_web', 'local_aiquiz_gen'),
        'group2_title' => get_string('wizard_use_course_content', 'local_aiquiz_gen'),
        'source_activities' => get_string('source_activities', 'local_aiquiz_gen'),
        'source_activities_desc' => get_string('source_activities_desc', 'local_aiquiz_gen'),
        'wizard_recommended' => get_string('wizard_recommended', 'local_aiquiz_gen'),
        'source_scan_course' => get_string('wizard_scan_entire_course', 'local_aiquiz_gen'),
        'source_scan_course_desc' => get_string('wizard_scan_entire_course_desc', 'local_aiquiz_gen'),
        'source_scan_resources' => get_string('wizard_scan_all_resources', 'local_aiquiz_gen'),
        'source_scan_resources_desc' => get_string('wizard_scan_all_resources_desc', 'local_aiquiz_gen'),
        'source_scan_activities' => get_string('wizard_scan_all_activities', 'local_aiquiz_gen'),
        'source_scan_activities_desc' => get_string('wizard_scan_all_activities_desc', 'local_aiquiz_gen'),
        'selected_sources' => get_string('selected_sources', 'local_aiquiz_gen'),
        'manual_text_entry' => get_string('manual_text_entry', 'local_aiquiz_gen'),
        'manual_text_entry_help' => get_string('manual_text_entry_help', 'local_aiquiz_gen'),
        'manual_text_placeholder' => get_string('manual_text_placeholder', 'local_aiquiz_gen'),
        'upload_files' => get_string('upload_files', 'local_aiquiz_gen'),
        'upload_files_help' => get_string('upload_files_help', 'local_aiquiz_gen'),
        'choose_files' => get_string('choose_files', 'local_aiquiz_gen'),
        'no_files_selected' => get_string('wizard_no_files_selected', 'local_aiquiz_gen'),
        'supported_formats' => get_string('supported_formats', 'local_aiquiz_gen'),
        'php_limits' => get_string('wizard_php_limits', 'local_aiquiz_gen', $phplimits),
        'url_label' => get_string('wizard_extract_from_url', 'local_aiquiz_gen'),
        'url_help' => get_string('wizard_enter_urls_help', 'local_aiquiz_gen'),
        'url_placeholder' => get_string('wizard_url_placeholder', 'local_aiquiz_gen'),
        'url_per_line_help' => get_string('wizard_url_per_line_help', 'local_aiquiz_gen'),
        'select_activities' => get_string('select_activities', 'local_aiquiz_gen'),
        'select_activities_help' => get_string('select_activities_help', 'local_aiquiz_gen'),
        'has_activities' => !empty($activitydata),
        'activities' => $activitydata,
        'activities_count_text' => get_string('wizard_activities_available', 'local_aiquiz_gen', count($activitydata)),
        'select_all' => get_string('select_all', 'local_aiquiz_gen'),
        'deselect_all' => get_string('deselect_all_topics', 'local_aiquiz_gen'),
        'n_activities_selected' => get_string('wizard_n_activities_selected', 'local_aiquiz_gen', 0),
        'noactivities' => get_string('noactivities', 'local_aiquiz_gen'),
        'cancel_url' => (new moodle_url('/course/view.php', ['id' => $courseid]))->out(false),
        'cancel' => get_string('cancel'),
        'next' => get_string('next'),
    ];

    // JavaScript handled by AMD module local_aiquiz_gen/wizard.

    return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step1', $context);
}
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step2(int $courseid, int $requestid): string {
    global $DB, $PAGE, $CFG, $OUTPUT;

    debugging("AI Step 2: max_execution_time=" . ini_get('max_execution_time') .
    ...
    if ($requestid === 0) {
        redirect(
            ...
            get_string('wizard_please_start_step1', 'local_aiquiz_gen'),
            null,
            \core\output\notification::NOTIFY_WARNING
        );
        return '';
    ...
    if ($request->status === 'pending') {
        try {
            ...
            $errors[] = get_string('wizard_could_not_update_status', 'local_aiquiz_gen', $e->getMessage());
        }
    }
    ...
    if ($request->status === 'analyzing') {
        ...
        debugging("AI Step 2 content extraction START", DEBUG_DEVELOPER);
        $allcontent = '';
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                ...
                try {
                    ...
                    $result = \local_aiquiz_gen\content_extractor::extract_from_file($filepath, $filename);
            ...
            foreach ($sources as $source) {
                ...
                if (strpos($source, 'course_activities:') === 0) {
                    ...
                    if (!empty($activityids)) {
                        try {
                            $activitycontent = \local_aiquiz_gen\content_extractor::extract_from_activities(
                                $courseid,
                                $activityids
                            );
                    ...
                    try {
                        $scanner = new \local_aiquiz_gen\course_scanner();
                        ...
                        $errors[] = get_string('wizard_scan_course_failed', 'local_aiquiz_gen', $e->getMessage());
                    }
                    ...
                    try {
                        ...
                        $errors[] = get_string('wizard_scan_resources_failed', 'local_aiquiz_gen', $e->getMessage());
                    }
                    ...
                    try {
                        ...
                        $errors[] = get_string('wizard_bulk_scan_failed', 'local_aiquiz_gen', $e->getMessage());
                    }
                }
            }
        }
        ...
        debugging(
            "AI Step 2 content extraction DONE in {$extractionduration}s: {$contentlen} bytes, {$contentwords} words",
            DEBUG_DEVELOPER
        );
        ...
        if (!empty(trim($allcontent))) {
            ...
            try {
                ...
                debugging("AI Step 2 topic analysis START: content_size={$contentsize} bytes" .
                    ($wastruncated ? " (TRUNCATED from original)" : ""), DEBUG_DEVELOPER);

                $analyzer = new \local_aiquiz_gen\topic_analyzer();
                ...
                debugging("AI Step 2 topic analysis DONE in {$analyzeduration}s: {$topiccount} topics found", DEBUG_DEVELOPER);
            } catch (Exception $e) {
                $analyzeduration = round(microtime(true) - ($analyzestart ?? microtime(true)), 2);
                debugging(
                    "AI Step 2 topic analysis FAILED after {$analyzeduration}s: " .
                    $e->getMessage() . " | " . $e->getTraceAsString(),
                    DEBUG_DEVELOPER
                );
                $errors[] = get_string('error:analysisfailed', 'local_aiquiz_gen') . ': ' . $e->getMessage();
            }

            // Update request status to topics_ready (separate try so topics aren't lost).
            if (empty($errors)) {
                try {
                    \local_aiquiz_gen\api::update_request_status($requestid, 'topics_ready');
                } catch (Exception $e) {
                    debugging('AI update_request_status FAILED: ' . $e->getMessage(), DEBUG_DEVELOPER);
                }
            }
        } else {
            // Only report error if no topics already exist (e.g. from deduplication cloning).
            $existingtopics = $DB->count_records('local_aiquiz_gen_topics', ['requestid' => $requestid]);
            if (empty($existingtopics)) {
                $errors[] = get_string('error:nocontent', 'local_aiquiz_gen');
            }
        }
    }

    // Display topics if available.
    $topics = $DB->get_records('local_aiquiz_gen_topics', ['requestid' => $requestid], 'level ASC, id ASC');
    ...
    if (!empty($topics)) {
        ...
        foreach ($topics as $topic) {
            ...
            if ($topic->title !== $cleanedtitle) {
                ...
                $DB->set_field('local_aiquiz_gen_topics', 'title', $cleanedtitle, ['id' => $topic->id]);
            }
        ...
        if (!empty($duplicateids)) {
            foreach ($duplicateids as $dupid) {
                $DB->delete_records('local_aiquiz_gen_topics', ['id' => $dupid]);
            }
        }
    ...
    // Prepare template context.
    $context = [
        'step2_title' => get_string('step2_title', 'local_aiquiz_gen'),
        'step2_description' => get_string('step2_description', 'local_aiquiz_gen'),
        'errors' => array_map(function ($msg) {
            return ['message' => $msg];
        }, $errors),
        'has_topics' => $hastopics,
        'analyzing' => !$hastopics,
        'analyzing_content' => get_string('analyzing_content', 'local_aiquiz_gen'),
    ];
    ...
    if ($hastopics) {
        ...
        $context['topics_found'] = get_string('topics_found', 'local_aiquiz_gen');
        $context['topics_select_help'] = get_string('topics_select_help', 'local_aiquiz_gen');
        $context['select_all_topics'] = get_string('select_all_topics', 'local_aiquiz_gen');
        $context['deselect_all_topics'] = get_string('deselect_all_topics', 'local_aiquiz_gen');
        $context['topics_discovered'] = get_string('wizard_topics_discovered', 'local_aiquiz_gen', count($topics));
        $context['n_topics_selected'] = get_string('wizard_n_topics_selected', 'local_aiquiz_gen', 0);
        ...
        $topicsselectedstr = json_encode(get_string('wizard_n_topics_selected', 'local_aiquiz_gen', '__COUNT__'));
        $PAGE->requires->js_amd_inline("
            require(['jquery'], function($) {
                ...
                function update_selected_count() {
                    var count = $('input.aiquiz-topic-checkbox:checked').length;
                    ...
                    if (count > 0) {
                        $('.aiquiz-topics-selected-bar').addClass('has-selection');
                    } else {
                        $('.aiquiz-topics-selected-bar').removeClass('has-selection');
                    }

                    // Update card selected states.
                    $('input.aiquiz-topic-checkbox').each(function() {
                        if ($(this).prop('checked')) {
                            $(this).closest('.aiquiz-topic-card').addClass('is-selected');
                        } else {
                            $(this).closest('.aiquiz-topic-card').removeClass('is-selected');
                        }
                    });
                }

                $('#select-all-topics').on('click', function() {
                    $('input.aiquiz-topic-checkbox').prop('checked', true);
                    update_selected_count();
                });
                $('#deselect-all-topics').on('click', function() {
                    $('input.aiquiz-topic-checkbox').prop('checked', false);
                    update_selected_count();
                });

                // Update on individual checkbox change.
                $(document).on('change', 'input.aiquiz-topic-checkbox', function() {
                    update_selected_count();
                });
    ...
    return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step2', $context);
}
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step3(int $courseid, int $requestid): string {
    global $DB, $OUTPUT;
    ...
    // Prepare template context.
    $context = [
        'step3_title' => get_string('step3_title', 'local_aiquiz_gen'),
        'step3_description' => get_string('step3_description', 'local_aiquiz_gen'),
        'no_topics' => empty($selectedtopics),
    ];

    if (empty($selectedtopics)) {
        $context['no_topics_message'] = get_string('wizard_no_topics_go_back', 'local_aiquiz_gen');
        $context['back_to_step2_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'requestid' => $requestid, 'step' => 2,
        ]))->out(false);
        $context['back_to_step2'] = get_string('wizard_back_to_step2', 'local_aiquiz_gen');

        return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step3', $context);
    }
    ...
    if (count($topicnames) > 3) {
        $topicspreview .= ' +' . get_string('wizard_n_more', 'local_aiquiz_gen', (count($topicnames) - 3));
    }
    $context['topics_info_html'] = html_writer::div(
        html_writer::tag(
            'span',
            '<i class="fa fa-info-circle aiquiz-icon-info"></i> ',
            ['class' => 'aiquiz-info-icon']
        ) .
        html_writer::tag(
            'span',
            get_string('wizard_n_topics_selected_colon', 'local_aiquiz_gen', count($selectedtopics)),
            ['class' => 'aiquiz-info-label']
        ) .
        html_writer::tag('span', $topicspreview, ['class' => 'aiquiz-info-topics']),
        'aiquiz-topics-info-bar'
    );
    ...
    $context['total_questions_label'] = get_string('total_questions', 'local_aiquiz_gen');

    // Difficulty options.
    $context['question_difficulty_label'] = get_string('question_difficulty', 'local_aiquiz_gen');
    $context['difficulty_options'] = [
        [
            'value' => 'easy_only',
            'label' => get_string('diff_easy', 'local_aiquiz_gen'),
            'css_class' => 'is-easy',
            'is_checked' => false,
        ],
        [
            'value' => 'balanced',
            'label' => get_string('wizard_balanced', 'local_aiquiz_gen'),
            'css_class' => 'is-balanced',
            'is_checked' => true,
        ],
        [
            'value' => 'hard_only',
            'label' => get_string('diff_hard', 'local_aiquiz_gen'),
            'css_class' => 'is-hard',
            'is_checked' => false,
        ],
    ];
    ...
    // Question types.
    $context['question_types_label'] = get_string('question_types', 'local_aiquiz_gen');
    $context['qtype_count_hint'] = get_string('wizard_qtype_count_hint', 'local_aiquiz_gen');
    $questiontypes = [
        'multichoice' => [
            'label' => get_string('qtype_multichoice', 'local_aiquiz_gen'),
            'icon' => '<i class="fa fa-dot-circle-o"></i>', 'color' => '#3B82F6',
        ],
        'truefalse' => [
            'label' => get_string('qtype_truefalse', 'local_aiquiz_gen'),
            'icon' => '<i class="fa fa-check"></i>', 'color' => '#10B981',
        ],
        'shortanswer' => [
            'label' => get_string('qtype_shortanswer', 'local_aiquiz_gen'),
            'icon' => '<i class="fa fa-pencil"></i>', 'color' => '#F59E0B',
        ],
        'essay' => [
            'label' => get_string('qtype_essay', 'local_aiquiz_gen'),
            'icon' => '<i class="fa fa-file-text-o"></i>', 'color' => '#64748B',
        ],
        'scenario' => [
            'label' => get_string('wizard_qtype_scenario', 'local_aiquiz_gen'),
    ...
    $context['total_label'] = get_string('wizard_total', 'local_aiquiz_gen');

    // Bloom's Taxonomy.
    $context['blooms_taxonomy_label'] = get_string('blooms_taxonomy', 'local_aiquiz_gen');
    $context['blooms_hint'] = get_string('wizard_blooms_hint', 'local_aiquiz_gen');
    $bloomslevels = [
        'remember' => ['label' => get_string('bloom_remember', 'local_aiquiz_gen'), 'default' => 20, 'color' => '#EF4444'],
        'understand' => ['label' => get_string('bloom_understand', 'local_aiquiz_gen'), 'default' => 25, 'color' => '#F59E0B'],
        'apply' => ['label' => get_string('bloom_apply', 'local_aiquiz_gen'), 'default' => 25, 'color' => '#10B981'],
        'analyze' => ['label' => get_string('bloom_analyze', 'local_aiquiz_gen'), 'default' => 15, 'color' => '#3B82F6'],
        'evaluate' => ['label' => get_string('bloom_evaluate', 'local_aiquiz_gen'), 'default' => 10, 'color' => '#8B5CF6'],
        'create' => ['label' => get_string('bloom_create', 'local_aiquiz_gen'), 'default' => 5, 'color' => '#EC4899'],
    ];
    $bloomsdata = [];
    ...
    $context['previous'] = get_string('previous');
    $context['generate_questions'] = get_string('generate_questions', 'local_aiquiz_gen');

    // Loading overlay strings.
    $context['generating_title'] = get_string('wizard_generating_questions_ellipsis', 'local_aiquiz_gen');
    $context['generating_subtitle'] = get_string('wizard_generating_please_wait', 'local_aiquiz_gen');
    $context['generating_warning'] = get_string('wizard_do_not_close', 'local_aiquiz_gen');
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step3_5(int $courseid, int $requestid): string {
    global $DB, $OUTPUT;
    ...
    $context = [
        ...
        'generating_questions' => get_string('generating_questions', 'local_aiquiz_gen'),
    ];

    if ($request->status === 'failed') {
        $context['failed_message'] = get_string('generation_failed', 'local_aiquiz_gen') . ': ' .
            s($request->error_message ?? get_string('error:unknown', 'local_aiquiz_gen'));
        $context['start_over_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'step' => 1,
        ]))->out(false);
        $context['wizard_start_over'] = get_string('wizard_start_over', 'local_aiquiz_gen');
    } else {
        $context['generating_questions_desc'] = get_string('generating_questions_desc', 'local_aiquiz_gen');
        $context['progress'] = (int)($request->progress ?? 0);
        $context['statusmessage'] = s($request->status_message ?? get_string('wizard_processing', 'local_aiquiz_gen'));
    }

    // JavaScript handled by AMD module local_aiquiz_gen/wizard.

    return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step3_5', $context);
}
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step4(int $courseid, int $requestid): string {
    ...
    // Prepare template context.
    $context = [
        'step4_title' => '<i class="fa fa-clipboard aiquiz-icon-primary"></i> ' .
            get_string('step4_title', 'local_aiquiz_gen'),
        'step4_description' => get_string('step4_description', 'local_aiquiz_gen'),
        'is_pending' => false,
        'is_failed' => false,
        'no_questions' => false,
        'has_questions' => false,
    ...
    if ($request->status === 'pending') {
        ...
        return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step4', $context);
    }

    // Failed status.
    if ($request->status === 'failed') {
        $context['is_failed'] = true;
        $context['failed_message'] = get_string('generation_failed', 'local_aiquiz_gen') . ': ' . $request->error_message;
        $context['retry_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'requestid' => $requestid, 'step' => 3,
        ]))->out(false);
        $context['retry'] = get_string('retry', 'local_aiquiz_gen');

        return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step4', $context);
    }

    // Get generated questions and sort by type (grouped) then by id within each type.
    $questions = $DB->get_records('local_aiquiz_gen_questions', ['requestid' => $requestid], 'id ASC');
    $typeorder = ['multichoice' => 1, 'truefalse' => 2, 'shortanswer' => 3, 'essay' => 4, 'matching' => 5, 'scenario' => 6];
    ...
    if (empty($questions)) {
        ...
        $context['no_questions_message'] = get_string('no_questions_generated', 'local_aiquiz_gen');
    ...
    // Labels.
    $context['total_upper'] = get_string('wizard_total_upper', 'local_aiquiz_gen');
    $context['approved_label'] = get_string('approved', 'local_aiquiz_gen');
    $context['pending_label'] = get_string('pending', 'local_aiquiz_gen');
    $context['rejected_label'] = get_string('rejected', 'local_aiquiz_gen');
    $context['select_all_label'] = get_string('select_all', 'local_aiquiz_gen');
    $context['bulk_action_label'] = get_string('bulk_action', 'local_aiquiz_gen');
    $context['approve_selected'] = get_string('approve_selected', 'local_aiquiz_gen');
    $context['reject_selected'] = get_string('reject_selected', 'local_aiquiz_gen');
    $context['delete_selected'] = get_string('delete_selected', 'local_aiquiz_gen');
    $context['apply_label'] = get_string('apply', 'local_aiquiz_gen');

    // Approve all button.
    $approveallurl = new moodle_url('/local/aiquiz_gen/wizard.php', [
        'courseid' => $courseid,
        'requestid' => $requestid,
    ...
    $context['approve_all_label'] = get_string('wizard_approve_all_questions', 'local_aiquiz_gen');

    // Filter labels.
    $context['filter_all_status'] = get_string('wizard_all_status', 'local_aiquiz_gen');
    $context['filter_all_types'] = get_string('wizard_all_types', 'local_aiquiz_gen');
    $context['filter_all_difficulty'] = get_string('wizard_all_difficulty', 'local_aiquiz_gen');
    $context['qtype_multichoice'] = get_string('qtype_multichoice', 'local_aiquiz_gen');
    $context['qtype_truefalse'] = get_string('qtype_truefalse', 'local_aiquiz_gen');
    $context['qtype_shortanswer'] = get_string('qtype_shortanswer', 'local_aiquiz_gen');
    $context['qtype_essay'] = get_string('qtype_essay', 'local_aiquiz_gen');
    $context['diff_easy'] = get_string('diff_easy', 'local_aiquiz_gen');
    $context['diff_medium'] = get_string('diff_medium', 'local_aiquiz_gen');
    $context['diff_hard'] = get_string('diff_hard', 'local_aiquiz_gen');
    ...
    $diffstringmap = [
        'easy' => get_string('diff_easy', 'local_aiquiz_gen'),
        'medium' => get_string('diff_medium', 'local_aiquiz_gen'),
        'hard' => get_string('diff_hard', 'local_aiquiz_gen'),
    ];

    // Status string map.
    $statusstringmap = [
        'approved' => get_string('approved', 'local_aiquiz_gen'),
        'rejected' => get_string('rejected', 'local_aiquiz_gen'),
        'pending' => get_string('pending', 'local_aiquiz_gen'),
    ];

    $maxregens = get_config('local_aiquiz_gen', 'max_regenerations') ?: 5;
    $questionnumber = 0;
    $questionsdata = [];
    ...
    foreach ($questions as $question) {
        ...
        $qdata = [
            ...
            'question_number_text' => get_string('wizard_question_number', 'local_aiquiz_gen', $questionnumber),
            ...
            'approved_label' => get_string('approved', 'local_aiquiz_gen'),
        ];
        ...
        if ($qdata['is_essay_or_scenario']) {
            ...
            if (!empty($answers)) {
                ...
                foreach ($answers as $answer) {
                    ...
                    $answerclass = 'answer-row aiquiz-answer-row';
                    $answerclass .= $iscorrect ? ' aiquiz-answer-correct' : ' aiquiz-answer-neutral';
        ...
        if ($qdata['not_approved']) {
            $qdata['approve_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
                'courseid' => $courseid,
                'requestid' => $requestid,
            ...
            $qdata['approve_label'] = get_string('approve_question', 'local_aiquiz_gen');
        }

        if ($qdata['not_rejected']) {
            $qdata['reject_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
                'courseid' => $courseid,
                'requestid' => $requestid,
            ...
            $qdata['reject_label'] = get_string('wizard_reject', 'local_aiquiz_gen');
        }

        $remainingregens = $maxregens - ($question->regeneration_count ?? 0);
        $qdata['has_regenerations'] = ($remainingregens > 0);
        if ($remainingregens > 0) {
            $qdata['regen_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
                'courseid' => $courseid,
                'requestid' => $requestid,
            ...
            $qdata['regen_label'] = get_string('regenerate_question', 'local_aiquiz_gen') .
                ' (' . $remainingregens . ')';
        }
    ...
    $context['next_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
 ...
 * @return string HTML
 */
function local_aiquiz_gen_render_step5(int $courseid, int $requestid): string {
    global $DB, $OUTPUT;
    ...
    $request = $DB->get_record('local_aiquiz_gen_requests', ['id' => $requestid]);

    // Count approved questions only.
    $approvedcount = $DB->count_records('local_aiquiz_gen_questions', [
        'requestid' => $requestid,
        'status' => 'approved',
    ]);
    $totalcount = $DB->count_records('local_aiquiz_gen_questions', ['requestid' => $requestid]);

    // Prepare template context.
    $context = [
        'step5_title' => get_string('step5_title', 'local_aiquiz_gen'),
        'step5_description' => get_string('step5_description', 'local_aiquiz_gen'),
    ...
    if ($totalcount == 0) {
        $context['no_questions_message'] = get_string('wizard_no_questions_go_back_step3', 'local_aiquiz_gen');
        $context['back_to_step3_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'requestid' => $requestid, 'step' => 3,
        ]))->out(false);
        $context['back_to_step3'] = get_string('wizard_back_to_step3', 'local_aiquiz_gen');
    } else if ($approvedcount == 0) {
        $context['no_approved_message'] = get_string('wizard_none_approved_yet', 'local_aiquiz_gen', $totalcount);
        $context['back_to_step4_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'requestid' => $requestid, 'step' => 4,
        ]))->out(false);
        $context['back_to_step4'] = get_string('wizard_back_to_step4', 'local_aiquiz_gen');
    } else {
        $readyinfo = new stdClass();
        $readyinfo->approved = $approvedcount;
        $readyinfo->total = $totalcount;
        $context['ready_deploy_message'] = get_string('wizard_approved_ready_deploy', 'local_aiquiz_gen', $readyinfo);
        $context['form_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'requestid' => $requestid, 'action' => 'deploy_questions',
        ]))->out(false);
        $context['sesskey'] = sesskey();
        $context['deployment_options'] = get_string('deployment_options', 'local_aiquiz_gen');
        $context['create_new_quiz'] = get_string('create_new_quiz', 'local_aiquiz_gen');
        $context['create_quiz_desc'] = get_string('wizard_create_quiz_desc', 'local_aiquiz_gen');
        $context['quiz_name_label'] = get_string('quiz_name', 'local_aiquiz_gen');
        $context['default_quiz_name'] = get_string('wizard_default_quiz_name', 'local_aiquiz_gen') . ' - ' . date('Y-m-d');
        $context['export_to_qbank'] = get_string('export_to_question_bank', 'local_aiquiz_gen');
        $context['qbank_desc'] = get_string('wizard_qbank_desc', 'local_aiquiz_gen');
        $context['category_name_label'] = get_string('category_name', 'local_aiquiz_gen');
        $defaultcategoryname = (!empty($request) && !empty($request->category_name))
            ? $request->category_name
            : get_string('wizard_ai_generated_questions', 'local_aiquiz_gen');
        $context['default_category_name'] = $defaultcategoryname;
        $context['category_help'] = get_string('wizard_category_help', 'local_aiquiz_gen');
        $context['prev_url'] = (new moodle_url('/local/aiquiz_gen/wizard.php', [
            'courseid' => $courseid, 'requestid' => $requestid, 'step' => 4,
        ]))->out(false);
        $context['previous'] = get_string('previous');
        $context['deploy_questions'] = get_string('deploy_questions', 'local_aiquiz_gen');
    }

    // JavaScript handled by AMD module local_aiquiz_gen/wizard.

    return $OUTPUT->render_from_template('local_aiquiz_gen/wizard_step5', $context);
}
 ...
 */
function local_aiquiz_gen_check_plugin_dependencies(): array {
    $errors = [];
    ...
    try {
        ...
        $errors[] = get_string('wizard_gateway_check_failed', 'local_aiquiz_gen', $e->getMessage());
    }
